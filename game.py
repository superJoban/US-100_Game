...
class Ship(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        #Calculate the distance of your hand from the senso
        sensor.TX_set()
        time.sleep(0.0001)
        sensor.TX_clear()
        ...
        timePeriod = t2 - t1
        distance = (timePeriod * 34300)/2
            
        #If distance is less than 25, then move left
        if (distance < 25):
            self.rect.move_ip(-5, 0)
        #If distance is greater than 35, then move right
        if (distance > 35):
            self.rect.move_ip(5, 0)
